...
export default {
    ...
    async getCurrentStudyTime({commit, dispatch}){
        await studyTimeService.getCurrentStudyTime()
            .then(response => {
                commit('SET_CURRENT_STUDY_TIME', response.data.data);
            })
            .catch((error) => {
                if (error.errors && Object.values(error.errors).length > 0) {
                    dispatch("alert/alertError", Object.values(error.errors)[0][0], { root: true });
    ...
    updateFacultyTimeSetting({commit, dispatch}, data){
        return studyTimeService.updateFacultyTimeSetting(data.id, data)
            .then(response => {
                dispatch('alert/alertSuccess',response.data.message, { root: true })
            })
            .catch((error) => {
